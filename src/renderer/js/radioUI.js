...
function loadStations() {
  ...
  radioStations.forEach(station => grid.appendChild(createStationCard(station, lang)));
  setupStationCards();
}
...
function createStationCard(station, lang) {
  ...
  card.innerHTML = `
    <div class="station-card-image"><img src="${station.image}" alt="${name}" loading="lazy"></div>
...
function setupStationCards() {
  document.querySelectorAll('.station-card').forEach(card => {
    ...
    card.addEventListener('click', e => { if (!e.target.closest('.station-play-btn')) playStation(id); });
    const btn = card.querySelector('.station-play-btn');
    if (btn) btn.addEventListener('click', e => { e.stopPropagation(); playStation(id); });
  });
}
...
function playStation(stationId) {
  ...
  if (!audioPlayer) { audioPlayer = new Audio(); setupAudioEvents(); }
...
function togglePlayPause() {
  if (!audioPlayer || !currentStation) { showToast(t('selectStationFirst'), 'error'); return; }
...
function setupAudioEvents() {
  if (!audioPlayer) return;
  audioPlayer.addEventListener('error', () => { showToast(t('playbackError'), 'error'); showLoading(false); stopPlayback(); });
  audioPlayer.addEventListener('ended', stopPlayback);
  audioPlayer.addEventListener('waiting', () => { showLoading(true); setVinylSpinning(false); });
  audioPlayer.addEventListener('playing', () => { showLoading(false); if (isPlaying) setVinylSpinning(true); });
}
...
function updateVolumeIcon() {
  ...
  if (!icon) return;
  icon.className = (volume === 0 || isMuted) ? 'fas fa-volume-mute'
                 : volume < 50              ? 'fas fa-volume-down'
                 :                            'fas fa-volume-up';
}
...
function setupPlayerControls() {
  ...
  if (vol) vol.addEventListener('input', e => setVolume(parseInt(e.target.value)));
}
...
function setSoundBars(active) {
  document.getElementById('playingAnimation')?.classList.toggle('active', active);
}
...
function updateUIForPlayingStation(station) {
  ...
  if (imgEl)  { imgEl.src = station.image; imgEl.style.display = 'block'; }
...
function updateCardPlayBtns() {
  ...
  document.querySelectorAll('.station-card').forEach(card => {
    const btn = card.querySelector('.station-play-btn i');
    if (!btn) return;
    btn.className = (card.dataset.station === currentStation.id && isPlaying)
