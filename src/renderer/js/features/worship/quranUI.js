...
function initQuranPage() {
  ...
  if (backBtn) {
    backBtn.addEventListener("click", () => {
      // Navigate back at the current screen size, not a hardcoded small size.
...
function updateQuranUI() {
  ...
  if (quranFullscreenBtn) {
    quranFullscreenBtn.setAttribute(
      "aria-label",
...
/**
 * Listen for connection recovery and reload Quran data
 */
window.addEventListener("connection-restored", () => {
  // Reload current surah
  if (window.currentSurahIndex !== undefined) {
    loadSurah(window.currentSurahIndex);
  }
});
